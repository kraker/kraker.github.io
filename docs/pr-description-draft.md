...
- **Content**:
  - `content/_index.md` — **draft rewrite calibrated to current posture: employment-forward, Field Manual held back, consulting/Merrick not advertised** (author still at Peraton). Frontmatter `draft: true`. Hero is identity-only (`Linux security engineer. / Red Hat ecosystem practitioner.`) with the v1.5 niche line as subtitle. CTAs point to Blog and `#study-guides` (the Field Manual hero badge + CTA have been removed — draft isn't far enough along to justify front-and-center marketing). New `## Study guides` section lists the RHCSA and RHCE GitHub Pages sites as the visible artifacts. `## Work with me` replaces "For hire" — LinkedIn + email as the route; no partner channel named. **⚠ Flip `draft: false` before merging** or `/` 404s on the live site.
  - `content/about/index.md` — **draft rewrite matched to the homepage posture**. Frontmatter `draft: true`. Identity lede unchanged. Body paragraph de-emphasizes the Field Manual (one trailing sentence, no CTA) and leads with the shipping study guides instead. `## Work with me` block mirrors the homepage: hiring-forward, LinkedIn as the primary route, no partner-channel mention. Personal-life content still dropped per §9. **⚠ Flip `draft: false` before merging** or `/about/` 404s on the live site.
